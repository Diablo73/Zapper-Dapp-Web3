...
function Nfts({chain, wallet, filteredNfts, setFilteredNfts, nfts, setNfts}) {
	...
	useEffect(() => {
		...
		for (let i = 0; i < nfts.length; i++) {
			if (nfts[i].name.toLowerCase().includes(nameFilter.toLowerCase()) && idFilter.length === 0) {
				filNfts.push(nfts[i]);
			} else if (nfts[i].token_id.includes(idFilter) && nameFilter.length === 0) {
				filNfts.push(nfts[i]);
			} else if (nfts[i].token_id.includes(idFilter) && nfts[i].name.toLowerCase().includes(nameFilter.toLowerCase())) {
				filNfts.push(nfts[i]);
			}
		}
